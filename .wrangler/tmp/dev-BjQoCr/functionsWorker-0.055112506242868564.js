...
// .wrangler/tmp/bundle-DbRolc/checked-fetch.js
...
// .wrangler/tmp/pages-oCOgy6/functionsWorker-0.055112506242868564.mjs
...
var RYZUMI = "https://api.ryzumi.net/api";
var SIPUTZX = "https://api.siputzx.my.id/api";
var CORS2 = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Methods": "GET, OPTIONS",
  "Access-Control-Allow-Headers": "Content-Type",
...
async function fetchJSON(url, opts = {}, timeoutMs = 12e3) {
...
async function tryRyzumi(url, type) {
  const data = await fetchJSON(`${RYZUMI}/downloader/ytdl?url=${encodeURIComponent(url)}&type=${type}`, {}, 12e3);
...
__name(tryRyzumi, "tryRyzumi");
__name2(tryRyzumi, "tryRyzumi");
async function trySiputzx(url, type) {
  const data = await fetchJSON(`${SIPUTZX}/d/ytdlp?url=${encodeURIComponent(url)}&type=${type}`, {}, 14e3);
...
__name(trySiputzx, "trySiputzx");
__name2(trySiputzx, "trySiputzx");
async function tryY2mate(url, type) {
  ...
  if (isAudio) {
    ...
    const vl = links?.mp4 || {};
    chosen = Object.values(vl).find((v) => v.q === "720p" && v.k) || Object.values(vl).find((v) => v.q === "360p" && v.k) || Object.values(vl)[0];
  }
...
__name(tryY2mate, "tryY2mate");
__name2(tryY2mate, "tryY2mate");
async function onRequest2(context) {
  ...
  if (!url) {
    return new Response(JSON.stringify({ status: false, error: "Parameter ?url= wajib diisi" }), { status: 400, headers: CORS2 });
  }
  const isYT = /youtube\.com|youtu\.be/i.test(url);
  if (!isYT) {
    return new Response(JSON.stringify({ status: false, error: "Hanya URL YouTube yang didukung" }), { status: 400, headers: CORS2 });
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ status: true, ...r }), { status: 200, headers: CORS2 });
  } catch (e) {
    errors.push("ryzumi: " + e.message);
  }
  try {
    const r = await trySiputzx(url, dlType);
    return new Response(JSON.stringify({ status: true, ...r }), { status: 200, headers: CORS2 });
  } catch (e) {
    errors.push("siputzx: " + e.message);
  }
  try {
  ...
  return new Response(JSON.stringify({ status: false, error: errors.join(" | ") }), { status: 502, headers: CORS2 });
}
...
// .wrangler/tmp/bundle-DbRolc/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-DbRolc/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.055112506242868564.js.map
