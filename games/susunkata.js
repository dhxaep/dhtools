...
function scramble(word) {
  ...
  do {
    ...
    attempts++;
  } while (result === word.toUpperCase() && attempts < 10);
  return result;
}
...
function showSoal() {
  ...
  if (state.soalIndex >= state.soalList.length || state.lives <= 0) {
    showGameEnd(state, startSusunKata);
    return;
  }
  const soal = state.soalList[state.soalIndex];
  state.wrongAttemptsThisQuestion = 0;
  // soal: { soal (scrambled) or jawaban, jawaban, deskripsi? }
  const jawaban = soal.jawaban || soal.kata || '';
  const scrambled = soal.soal || scramble(jawaban);
  const body = document.getElementById('gameBody');

  // Render letter tiles
  body.innerHTML = `
    <p class="note-text">Soal ${state.soalIndex + 1}/${state.soalList.length}</p>
    <p class="game-question" style="font-size:2rem;letter-spacing:0.15em;font-family:var(--font-mono);color:var(--accent)">${scrambled}</p>
    ...
    <div class="game-input-wrap">
      <input class="game-input" id="skInput" type="text" placeholder="Susun jadi kata..." autocomplete="off" />
      <button class="btn-primary" id="skSubmit">Cek</button>
    </div>
  `;
  updateUI(state);
  ...
  const handle = () => {
    ...
    const correct = jawaban.toUpperCase().trim();
    if (ans === correct) {
      stopTimer();
      input.classList.add('correct');
      input.value = jawaban;
      input.disabled = true;
      document.getElementById('skSubmit').disabled = true;
      ...
      if (state.wrongAttemptsThisQuestion >= 5) {
        ...
        lucide.createIcons();
        state.soalIndex++;
        if (state.lives <= 0) { setTimeout(() => showGameEnd(state, startSusunKata), 1000); }
  ...
  document.getElementById('skSubmit')?.addEventListener('click', handle);
